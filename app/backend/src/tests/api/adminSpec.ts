...
describe('Admin Endpoints', () => {
  ...
  describe('Access control', () => {
    it('should reject requests without a token', async () => {
      const res = await request.get('/admin/users').expect(401);
    ...
    it('PUT /users/:id should ignore a role sent in the body', async () => {
      await request
        .put(`/users/${alice.userId}`)
        .set('Authorization', `Bearer ${alice.token}`)
        .send({ firstName: 'Alice', role: 'admin' })
        .expect(200);

      const me = await request.get('/auth/me').set('Authorization', `Bearer ${alice.token}`).expect(200);
      expect(me.body.data.role).toBe('customer');

      await request.get('/admin/users').set('Authorization', `Bearer ${alice.token}`).expect(403);
    });
  });
  ...
  describe('Users', () => {
    ...
    it('GET /admin/users should paginate with limit and offset', async () => {
      const first = await request.get('/admin/users?limit=1').set('Authorization', `Bearer ${admin.token}`).expect(200);
      const second = await request.get('/admin/users?limit=1&offset=1').set('Authorization', `Bearer ${admin.token}`).expect(200);
    ...
    it('GET /admin/users should reject an out-of-range limit', async () => {
      const res = await request.get('/admin/users?limit=101').set('Authorization', `Bearer ${admin.token}`).expect(400);
      expect(res.body.message).toBe('limit must be an integer between 1 and 100');
      await request.get('/admin/users?limit=0').set('Authorization', `Bearer ${admin.token}`).expect(400);
      await request.get('/admin/users?offset=-1').set('Authorization', `Bearer ${admin.token}`).expect(400);
    });

    it('GET /admin/users/:id should return any user with recentPurchases', async () => {
      const res = await request
        .get(`/admin/users/${alice.userId}`)
        .set('Authorization', `Bearer ${admin.token}`)
        .expect(200);
    ...
    it('GET /admin/users/:id should return 404 for an unknown id', async () => {
      await request.get('/admin/users/999999').set('Authorization', `Bearer ${admin.token}`).expect(404);
    });
    ...
    it('PUT /admin/users/:id/role should promote a customer and revoke their refresh tokens', async () => {
      const res = await request
        .put(`/admin/users/${bob.userId}/role`)
        .set('Authorization', `Bearer ${admin.token}`)
        .send({ role: 'admin' })
        .expect(200);
      expect(res.body.data.role).toBe('admin');
      ...
      await request.post('/auth/refresh').send({ refreshToken: bob.refreshToken }).expect(401);

      // A fresh login carries the new role
      const login = await request.post('/auth/login').send({ username: bob.username, password: bob.password }).expect(200);
      expect(login.body.data.user.role).toBe('admin');
      await request.get('/admin/users').set('Authorization', `Bearer ${login.body.data.accessToken}`).expect(200);
    ...
    it('PUT /admin/users/:id/role should demote an admin and block their existing access token immediately', async () => {
      ...
      await request.get('/admin/users').set('Authorization', `Bearer ${bob.token}`).expect(403);
    ...
    it('PUT /admin/users/:id/role should reject an invalid role', async () => {
      await request
        .put(`/admin/users/${alice.userId}/role`)
        .set('Authorization', `Bearer ${admin.token}`)
        .send({ role: 'root' })
        .expect(400);
    });
    ...
    it('PUT /admin/users/:id/role should not let an admin change their own role', async () => {
      const res = await request
        .put(`/admin/users/${admin.userId}/role`)
        .set('Authorization', `Bearer ${admin.token}`)
        .send({ role: 'customer' })
        .expect(400);
    ...
    it('DELETE /admin/users/:id should not let an admin delete their own account', async () => {
      await request.delete(`/admin/users/${admin.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(400);
    });
    ...
    it('DELETE /admin/users/:id should delete another user', async () => {
      ...
      await request.get(`/admin/users/${createdUserId}`).set('Authorization', `Bearer ${admin.token}`).expect(404);
    });
  ...
  describe('Orders', () => {
    ...
    it('POST /admin/orders should return 404 for an unknown user and 400 for a bad status', async () => {
      await request.post('/admin/orders').set('Authorization', `Bearer ${admin.token}`).send({ userId: 999999 }).expect(404);
      await request.post('/admin/orders').set('Authorization', `Bearer ${admin.token}`).send({ userId: alice.userId, status: 'shipped' }).expect(400);
      await request.post('/admin/orders').set('Authorization', `Bearer ${admin.token}`).send({}).expect(400);
    });

    it('GET /admin/orders should list orders across users and support filters', async () => {
      const all = await request.get('/admin/orders?limit=100').set('Authorization', `Bearer ${admin.token}`).expect(200);
      expect(all.body.data.map((o: { id: number }) => o.id)).toContain(orderId);

      const byUser = await request.get(`/admin/orders?userId=${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      ...
      const byStatus = await request.get('/admin/orders?status=active&limit=100').set('Authorization', `Bearer ${admin.token}`).expect(200);
      byStatus.body.data.forEach((o: { status: string }) => expect(o.status).toBe('active'));

      await request.get('/admin/orders?status=bogus').set('Authorization', `Bearer ${admin.token}`).expect(400);
      await request.get('/admin/orders?userId=abc').set('Authorization', `Bearer ${admin.token}`).expect(400);
    });

    it('GET /admin/orders/:id should return any order', async () => {
      const res = await request.get(`/admin/orders/${orderId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      expect(res.body.data.id).toBe(orderId);
      await request.get('/admin/orders/999999').set('Authorization', `Bearer ${admin.token}`).expect(404);
    });

    it('POST and GET /admin/orders/:id/products should manage products on any order', async () => {
      const add = await request
        .post(`/admin/orders/${orderId}/products`)
        .set('Authorization', `Bearer ${admin.token}`)
        .send({ productId, quantity: 2 })
        .expect(200);
      expect(add.body.data.orderId).toBe(orderId);

      const list = await request.get(`/admin/orders/${orderId}/products`).set('Authorization', `Bearer ${admin.token}`).expect(200);
    ...
    it('PUT /admin/orders/:id should update any order status', async () => {
      ...
      const own = await request.get(`/orders/${orderId}`).set('Authorization', `Bearer ${alice.token}`).expect(200);
      expect(own.body.data.status).toBe('complete');
    });

    it('admin may also use the customer order routes on any order', async () => {
      await request.get(`/orders/${orderId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      const filtered = await request.get(`/orders?userId=${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      filtered.body.data.forEach((o: { userId: number }) => expect(o.userId).toBe(alice.userId));
      await request.get(`/orders/user/${alice.userId}/completed`).set('Authorization', `Bearer ${admin.token}`).expect(200);
    });

    it('a customer still cannot reach another user\'s order', async () => {
      await request.get(`/orders/${orderId}`).set('Authorization', `Bearer ${bob.token}`).expect(404);
    });

    it('DELETE /admin/orders/:id should delete any order', async () => {
      await request.delete(`/admin/orders/${orderId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      await request.get(`/admin/orders/${orderId}`).set('Authorization', `Bearer ${admin.token}`).expect(404);
    });
  });
  ...
  describe('Carts', () => {
    ...
    it('POST /admin/carts/:userId should return 404 for an unknown user', async () => {
      await request.post('/admin/carts/999999').set('Authorization', `Bearer ${admin.token}`).send({ productId, quantity: 1 }).expect(404);
    });

    it('GET /admin/carts should list cart items across users and filter by userId', async () => {
      const all = await request.get('/admin/carts?limit=100').set('Authorization', `Bearer ${admin.token}`).expect(200);
      const mine = all.body.data.find((i: { id: number }) => i.id === cartItemId);
      expect(mine).toBeDefined();
      expect(mine.productName).toBe('Admin Spec Product');

      const byUser = await request.get(`/admin/carts?userId=${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
    ...
    it('GET /admin/carts/:userId should return one user\'s cart', async () => {
      const res = await request.get(`/admin/carts/${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
    ...
    it('GET and PUT /admin/cart-items/:id should read and update any cart item', async () => {
      const shown = await request.get(`/admin/cart-items/${cartItemId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      expect(shown.body.data.id).toBe(cartItemId);

      const updated = await request
        .put(`/admin/cart-items/${cartItemId}`)
        .set('Authorization', `Bearer ${admin.token}`)
        .send({ quantity: 5 })
        .expect(200);
      ...
      const own = await request.get('/cart').set('Authorization', `Bearer ${alice.token}`).expect(200);
      expect(own.body.data.find((i: { id: number }) => i.id === cartItemId).quantity).toBe(5);

      await request.put(`/admin/cart-items/${cartItemId}`).set('Authorization', `Bearer ${admin.token}`).send({ quantity: 0 }).expect(400);
    });

    it('DELETE /admin/cart-items/:id should remove any cart item', async () => {
      await request.delete(`/admin/cart-items/${cartItemId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      await request.get(`/admin/cart-items/${cartItemId}`).set('Authorization', `Bearer ${admin.token}`).expect(404);
    });

    it('DELETE /admin/carts/:userId should clear any user\'s cart', async () => {
      await request.post(`/admin/carts/${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).send({ productId, quantity: 1 }).expect(201);
      await request.delete(`/admin/carts/${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
  ...
  describe('Addresses', () => {
    ...
    it('POST /admin/addresses should validate userId and required fields', async () => {
      await request.post('/admin/addresses').set('Authorization', `Bearer ${admin.token}`).send({ fullName: 'X', address: 'Y', city: 'Z' }).expect(400);
      await request.post('/admin/addresses').set('Authorization', `Bearer ${admin.token}`).send({ userId: 999999, fullName: 'X', address: 'Y', city: 'Z' }).expect(404);
      await request.post('/admin/addresses').set('Authorization', `Bearer ${admin.token}`).send({ userId: alice.userId, fullName: 'X' }).expect(400);
    });

    it('GET /admin/addresses should list addresses across users and filter by userId', async () => {
      const all = await request.get('/admin/addresses?limit=100').set('Authorization', `Bearer ${admin.token}`).expect(200);
      expect(all.body.data.map((a: { id: number }) => a.id)).toContain(addressId);

      const byUser = await request.get(`/admin/addresses?userId=${alice.userId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
    ...
    it('GET and PUT /admin/addresses/:id should read and update any address', async () => {
      const shown = await request.get(`/admin/addresses/${addressId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      expect(shown.body.data.id).toBe(addressId);

      const updated = await request
        .put(`/admin/addresses/${addressId}`)
        .set('Authorization', `Bearer ${admin.token}`)
      ...
      // The owner sees the change through the customer routes
      const own = await request.get(`/addresses/${addressId}`).set('Authorization', `Bearer ${alice.token}`).expect(200);
      expect(own.body.data.city).toBe('Shelbyville');

      await request.put(`/admin/addresses/${addressId}`).set('Authorization', `Bearer ${admin.token}`).send({}).expect(400);
      await request.put('/admin/addresses/999999').set('Authorization', `Bearer ${admin.token}`).send({ city: 'X' }).expect(404);
    });

    it('DELETE /admin/addresses/:id should delete any address', async () => {
      await request.delete(`/admin/addresses/${addressId}`).set('Authorization', `Bearer ${admin.token}`).expect(200);
      await request.get(`/admin/addresses/${addressId}`).set('Authorization', `Bearer ${admin.token}`).expect(404);
    });
  });
