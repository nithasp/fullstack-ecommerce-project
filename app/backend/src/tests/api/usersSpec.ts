...
describe('User endpoints', () => {
  describe('GET /users/:id', () => {
    it('returns the caller with their recent purchases', async () => {
      const customer = await registerCustomer('profile');
    ...
    it('refuses another account', async () => {
      const customer = await registerCustomer('owner');
    ...
    it('refuses another account even for an admin token, which has /admin/users for that', async () => {
      ...
      const customer = await registerCustomer('target');
      await admin.get(`/users/${customer.id}`).expect(403);
    });

    it('ignores an admin role claimed by the token itself', async () => {
      const customer = await registerCustomer('claimer');
      const other = await registerCustomer('victim');
  ...
  describe('PATCH /users/:id', () => {
    it('updates profile fields', async () => {
      const customer = await registerCustomer('renamer');
    ...
    it('needs at least one field', async () => {
      const customer = await registerCustomer('emptyupdate');
    ...
    it('cannot change the role', async () => {
      const customer = await registerCustomer('promoter');
  ...
  describe('PUT /users/:id/password', () => {
    it('needs the current password', async () => {
      const customer = await registerCustomer('pwwrong');
      const res = await customer
    ...
    it('changes the password, ends other sessions and keeps this one signed in', async () => {
      const customer = await registerCustomer('pwchange');
    ...
    it('refuses to change another account', async () => {
      const customer = await registerCustomer('pwowner');
      const other = await registerCustomer('pwother');
      await customer
  ...
  describe('DELETE /users/:id', () => {
    it('closes the account, scrubs the profile and keeps the orders', async () => {
      ...
      const customer = await registerCustomer('closer');
