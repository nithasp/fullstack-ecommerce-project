...
export class CartRepository {
  ...
  async getAll(filters: { userId?: number }, page: Pagination): Promise<CartItem[]> {
    const params: number[] = [];
    let sql = `${SELECT_WITH_PRODUCT}${this.where(filters, params)}`;
  ...
  async count(filters: { userId?: number }): Promise<number> {
    const params: number[] = [];
    const { rows } = await pool.query(`SELECT COUNT(*) FROM cart_items ci${this.where(filters, params)}`, params);
    return parseInt(rows[0].count, 10);
  }

  // Admin: a cart item by id regardless of owner
  async showById(cartItemId: number): Promise<CartItem | null> {
    const { rows } = await pool.query(`${SELECT_WITH_PRODUCT} WHERE ci.id = $1`, [cartItemId]);
  ...
  private where(filters: { userId?: number }, params: number[]): string {
    ...
    return ` WHERE ci.user_id = $${params.length}`;
  }
