...
- The refresh cookie is `SameSite=Strict`, so the browser only sends it when the page and the API
  share a site. Give the frontend a domain next to the API's, e.g. `store.example.com` and
  `api.example.com`.
